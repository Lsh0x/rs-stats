...
mod tests {
    ...
    fn test_population_std_dev_integers() {
        ...
        assert!((result.unwrap() - expected).abs() < EPSILON, "Population std_dev for integers should be sqrt(2.0)");
    }
    ...
    fn test_population_std_dev_floats() {
        ...
        let result = std_dev(&data);
        let expected = 2.0_f64.sqrt();  // sqrt of population variance (2.0)
        assert!((result.unwrap() - expected).abs() < EPSILON, "Population std_dev for floats should be sqrt(2.0)");
    }
    ...
    fn test_population_std_dev_mixed_floats() {
        ...
        let result = std_dev(&data);
        let expected = 2.0_f64.sqrt();  // sqrt of population variance (2.0)
        assert!((result.unwrap() - expected).abs() < EPSILON, "Population std_dev for mixed floats should be sqrt(2.0)");
    }
}
